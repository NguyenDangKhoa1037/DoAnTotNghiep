...
public class bl_Joystick : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    void Start()
    {
        ...
        if (GetComponent<Image>() != null)
        {
            ...
            stickImage = StickRect_.GetComponent<Image>();
    ...
    void Update()
    {
        ...
        StickRect_.position = Vector3.SmoothDamp(StickRect_.position, DeathArea_, ref currentVelocity, smoothTime);
        //When is in default position, we not need continue update this
        if (Vector3.Distance(StickRect_.position, DeathArea_) < .1f)
        {
            isFree = false;
            isHolding = false;
            StickRect_.position = DeathArea_;
        }
    }
    ...
    public void OnDrag(PointerEventData data)
    {
        ...
        if (data.pointerId == lastId)
        {
            ...
            if (Vector2.Distance(DeathArea_, position) < radio)
            {
                StickRect_.position = position;
            }
            else
            {
                StickRect_.position = DeathArea_ + (position - DeathArea_).normalized * radio;
            }
        }
    }
    ...
    IEnumerator ScaleJoysctick(bool increase)
    {
        float _time = 0;
            ...
            {
                Vector3 v = StickRect_.localScale;
            if (increase)
            {
                v = Vector3.Lerp(StickRect_.localScale, PressScaleVector, (_time / Duration));
            }
            else
            {
                v = Vector3.Lerp(StickRect_.localScale, Vector3.one, (_time / Duration));
            }
            StickRect_.localScale = v;
    ...
    private float radio { get { return (Radio_ * 5 + Mathf.Abs((diff - CenterReference.position.magnitude))); } }
    ...
    public float Horizontal
    {
        get
        {
            return (StickRect_.position.x - DeathArea_.x) / Radio_;
        }
    }
    ...
    public float Vertical
    {
        get
        {
            return (StickRect_.position.y - DeathArea_.y) / Radio_;
        }
    }

    public bool IsHoliding
    {
        get { return isHolding; }
    }

    public float Radio_ { get => Radio;}
    public RectTransform StickRect_ { get => StickRect;}
    public Vector3 DeathArea_ { get => DeathArea; }
}